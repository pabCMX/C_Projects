...
static inline uint64_t presieve_word_at(size_t odd_index_mod) {
    ...
    return (presieve_words[word] >> shift) |
           (presieve_words[word + 1] << (64u - shift));
}
...
static void cross_off_prime(uint64_t *words, uint64_t low_index,
                            uint64_t high_index, uint64_t p,
                            uint64_t *cross_inout);

static void cross_presieved_primes(uint64_t *words, uint64_t low_index,
                                   uint64_t high_index) {
    static const uint32_t small_primes[] = {3, 5, 7, 11, 13, 17, 19};
...
static void init_cross_cursors(uint64_t range_start, uint64_t range_end,
                               const uint32_t *base_primes, size_t prime_begin,
                               size_t prime_end, uint64_t *next_cross) {
    size_t cursor = 0;
...
static void sieve_odd_segment(uint64_t low_index, uint64_t high_index,
                              const uint32_t *base_primes, size_t prime_begin,
                              size_t prime_end, uint64_t *words,
...
static void thread_prime_bounds(const uint32_t *base_primes,
                                size_t base_prime_count,
...
static void *sieve_worker(void *arg) {
    ...
    uint64_t *words =
        alloc_aligned(SEGMENT_ALIGNMENT, segment_words * sizeof *words);
    uint64_t *next_cross = active_prime_count == 0
                               ? NULL
                               : malloc(active_prime_count * sizeof *next_cross);
    ...
    init_cross_cursors(worker->range_start, worker->range_end, work->base_primes,
                       worker->prime_begin, worker->prime_end, next_cross);
    ...
    for (uint64_t low_index = worker->range_start; low_index < worker->range_end;) {
        ...
        sieve_odd_segment(low_index, high_index, work->base_primes,
                          worker->prime_begin, worker->prime_end, words,
...
static int run_prime_search(uint64_t end, int threads, bool verbose,
    ...
    for (int i = 0; i < threads; ++i) {
        ...
        thread_prime_bounds(base_primes, base_prime_count,
                            &args[i].prime_begin, &args[i].prime_end);
    }
