...
// v12 — Cache-aware segment sizing (placeholder from v9; implement the change below).
// Goal: choose block_size from L1/L2-oriented sizes (or a small sweep), not a fixed
//       wheel_period multiple. Keep marking algorithm the same as the parent version.
// Concept: CPU cache hierarchy (working set vs L1/L2; why L1 miss rate matters).
// Check: suite correctness; perf stat L1-dcache-load-misses vs wall time across sizes.
