...
static void odds_seg_sieve(uint32_t *primes, uint64_t base_prime_count, uint32_t *wheel_gaps,
                           uint32_t *strides, uint32_t live_count, uint32_t *prime_phase,
                           uint64_t *is_composite, uint64_t *next_cursor, uint64_t high_num,
                           uint64_t low_num, u128 *out_p_sum, uint64_t *out_p_count) {
  u128     segment_sum    = 0;
  ...
  for (uint64_t i = W_PRIME_COUNT + 1; i < active_prime_count; i++) {
    ...
    // And we finally fill a strides[] array with every gap multiple for the segment
    for (uint32_t j = 0; j < live_count; j++) {
      // We walk through the whole wheel_gap array
      strides[j] = p * wheel_gaps[j];
    }

    // Now while we are below the size of the segment, we stride by p and mark each multiple.
    while (offset < segment_size) {
      is_composite[offset >> 6] |= 1ULL << (offset & 63);
      // We just pull the next stride into the offset and keep going.
      offset += strides[phase];
      if (++phase == live_count)
        phase = 0;
    }
...
static int run_full_search(uint32_t *primes, uint64_t base_prime_count, uint64_t *pre_sieve,
  ...
  uint32_t *strides = calloc(live_count, sizeof(uint32_t));
  if (strides == NULL) {
    printf("Unable to allocate full sieve strides array. Exiting...\n");
    return EXIT_FAILURE;
  }
  ...
  for (uint32_t i = 0;; i++) {
    ...
    odds_seg_sieve(primes, base_prime_count, wheel_gaps, strides, live_count, prime_phase,
                   is_composite, next_cursor, high, low, prime_sum, total_primes_counter);
...
// v11 — Precomputed wheel strides.
// Goal: in the wheel mark loop, replace `offset += p * wheel_gaps[phase]` with a lookup
//       (per-prime or shared table of pre-multiplied strides). Keep hybrid/cutoff out of scope.
// Concept: time–memory tradeoff in a hot loop (lookup vs multiply / dependent load chain).
// Check: suite correctness; perf annotate — less time on imul/gap indexing in the mark loop.
