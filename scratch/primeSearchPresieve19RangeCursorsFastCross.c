...
enum {
  ...
  SEGMENT_ODD_COUNT_DEFAULT  = 1 << 22,
  /* Up to 8 MiB segments for large endpoints (2^56+). */
  SEGMENT_ODD_COUNT_MAX      = 1 << 26,
  SEGMENT_MAX_NWORDS         = (SEGMENT_ODD_COUNT_MAX + 63) / 64,
  PRESIEVE_LINEAR_WORDS      = PRESIEVE_NWORDS + SEGMENT_MAX_NWORDS + 1,
  DEFAULT_CURSOR_BUDGET_MIB  = 512,
};

static const uint32_t presieved_small_primes[] = {3, 5, 7, 11, 13, 17, 19};
...
static uint64_t presieve_linear[PRESIEVE_LINEAR_WORDS];
...
static void build_presieve_linear_stream(void) {
  size_t mod = 0;
  for (size_t w = 0; w < PRESIEVE_LINEAR_WORDS; ++w) {
    presieve_linear[w] = presieve_word_at(mod);
    mod += 64u;
...
static void build_presieve_table(void) {
  ...
  for (size_t i = 0; i < sizeof presieved_small_primes / sizeof presieved_small_primes[0]; ++i) {
    const size_t p     = presieved_small_primes[i];
  ...
  build_presieve_linear_stream();
  presieve_table_built = true;
}

static void apply_presieve_rotated(uint64_t *words, size_t nwords, size_t mod) {
...
static void apply_presieve(uint64_t *words, size_t size, uint64_t low_index) {
  ...
  const size_t mod    = (size_t)(low_index % PRESIEVE_PERIOD);

  if ((low_index & 63u) == 0u) {
    const size_t start = mod >> 6;
    if (start + nwords <= PRESIEVE_LINEAR_WORDS) {
      memcpy(words, &presieve_linear[start], nwords * sizeof *words);
      return;
    }
  }

  apply_presieve_rotated(words, nwords, mod);
}
...
static void cross_presieved_primes(uint64_t *words, uint64_t low_index, uint64_t high_index) {
  ...
  for (size_t i = 0; i < sizeof presieved_small_primes / sizeof presieved_small_primes[0]; ++i) {
    const uint64_t p         = presieved_small_primes[i];
    const uint64_t p_squared = p * p;
...
static size_t prime_limit_index(const uint32_t *base_primes, size_t prime_begin, size_t prime_end,
                                uint64_t high_exclusive) {
  size_t lo = prime_begin;
  size_t hi = prime_end;

  while (lo < hi) {
    const size_t   mid = lo + (hi - lo) / 2;
    const uint64_t p   = base_primes[mid];
    if (p * p >= high_exclusive) {
      hi = mid;
    } else {
      lo = mid + 1;
    }
...
static int grow_cross_cursors(cursor_bank_t *bank, size_t needed, uint64_t low_index,
                              uint64_t range_end, const uint32_t *base_primes,
                              size_t prime_begin) {
  if (needed <= bank->count) {
    return 0;
  }

  if (needed > bank->capacity) {
    size_t new_capacity = bank->capacity == 0 ? needed : bank->capacity;
    while (new_capacity < needed) {
      new_capacity *= 2;
    }

    uint64_t *next_cross = bank->next_cross == NULL
                               ? malloc(new_capacity * sizeof *next_cross)
                               : realloc(bank->next_cross, new_capacity * sizeof *next_cross);
    if (next_cross == NULL) {
      return -1;
    }

    bank->next_cross = next_cross;
    bank->capacity   = new_capacity;
  }

  for (size_t i = bank->count; i < needed; ++i) {
    const uint64_t p     = base_primes[prime_begin + i];
    const uint64_t first = first_cross_odd_index(low_index, p);
    bank->next_cross[i]  = first >= range_end ? cross_inactive : first;
  }
...
static void cross_off_prime(uint64_t *words, uint64_t low_index, uint64_t high_index, uint64_t p,
  ...
  const size_t rel_limit = (size_t)(high_index - low_index);
  size_t       rel       = (size_t)(cross - low_index);
  const size_t pstep     = (size_t)p;
...
static void sieve_odd_segment(uint64_t low_index, uint64_t high_index, const uint32_t *base_primes,
  ...
  const uint64_t high_exclusive = 2 * high_index + 1;
  const size_t   segment_prime_end =
      prime_limit_index(base_primes, prime_begin, prime_end, high_exclusive);

  size_t cursor = 0;
  for (size_t i = prime_begin; i < segment_prime_end; ++i, ++cursor) {
    const uint64_t cross = next_cross[cursor];
    if (cross == cross_inactive || cross >= high_index) {
      continue;
    }

    cross_off_prime(words, low_index, high_index, base_primes[i], &next_cross[cursor]);
  }

  sum_segment_primes(words, size, 2 * low_index + 1, sum_out, count_out);
}
...
static void thread_prime_bounds(const uint32_t *base_primes, size_t base_prime_count,
  ...
  *prime_end_out   = base_prime_count;
}
...
static void *sieve_worker(void *arg) {
  ...
  cursor_bank_t bank         = {0};

  if (words == NULL) {
    free(bank.next_cross);
  ...
  for (uint64_t low_index = worker->range_start; low_index < worker->range_end;) {
    ...
    const uint64_t high_exclusive    = 2 * high_index + 1;
    const size_t   segment_prime_end = prime_limit_index(
        work->base_primes, worker->prime_begin, worker->prime_end, high_exclusive);
    const size_t needed_cursors = segment_prime_end - worker->prime_begin;

    if (grow_cross_cursors(&bank, needed_cursors, low_index, worker->range_end, work->base_primes,
                           worker->prime_begin) != 0) {
      free(bank.next_cross);
      free_aligned(words);
      atomic_store(&work->failed, 1);
#ifdef _WIN32
    ...
    sieve_odd_segment(low_index, high_index, work->base_primes, worker->prime_begin,
                      segment_prime_end, words, bank.next_cross, &segment_sum, &segment_count);
...
static int physical_worker_slots(void) {
...
static uint64_t choose_segment_odd_count(uint64_t end) {
  if (end <= (1ULL << 42)) {
    return SEGMENT_ODD_COUNT_DEFAULT;
  }
  if (end <= (1ULL << 48)) {
    return 1ULL << 23;
  }
  ...
  return SEGMENT_ODD_COUNT_MAX;
}

static uint64_t cursor_budget_bytes(void) {
  const char *env = getenv("CURSOR_BUDGET_MIB");
  unsigned long long mib =
      env == NULL ? DEFAULT_CURSOR_BUDGET_MIB : strtoull(env, NULL, 10);
  if (mib == 0) {
    mib = DEFAULT_CURSOR_BUDGET_MIB;
  }
  return (uint64_t)mib * 1024ULL * 1024ULL;
}

static int choose_thread_count(size_t worst_case_cursors) {
  const int cores = physical_worker_slots();
  if (worst_case_cursors == 0) {
    return cores;
  }

  const uint64_t cursor_bytes = (uint64_t)worst_case_cursors * sizeof(uint64_t);
  const uint64_t budget       = cursor_budget_bytes();
  int            max_by_mem   = (int)(budget / cursor_bytes);
  if (max_by_mem < 1) {
    max_by_mem = 1;
  }

  return cores < max_by_mem ? cores : max_by_mem;
}
...
static int run_prime_search(uint64_t end, int threads, bool verbose, u128 *sum_out,
  ...
  const uint64_t segment_odd_count = choose_segment_odd_count(end);
  ...
  size_t prime_begin = 0;
  size_t prime_end   = 0;
  thread_prime_bounds(base_primes, base_prime_count, &prime_begin, &prime_end);
  const size_t worst_case_cursors =
      prime_limit_index(base_primes, prime_begin, prime_end, 2 * odd_index_limit + 1) - prime_begin;
  const int effective_threads = choose_thread_count(worst_case_cursors);
  if (threads > effective_threads) {
  ...
  if (verbose) {
    printf("Segment odd slots: %llu; workers: %d; cursor slots (worst): %zu\n",
           (unsigned long long)segment_odd_count, threads, worst_case_cursors);
  ...
  for (int i = 0; i < threads; ++i) {
    ...
    args[i].prime_end = prime_limit_index(base_primes, args[i].prime_begin, args[i].prime_end,
                                          2 * args[i].range_end + 1);
  }
...
int main(int argc, char *argv[]) {
  ...
  int threads = physical_worker_slots();
