...
static void sieve_odd_segment(uint64_t low_index, uint64_t high_index,
                              const uint32_t *base_primes,
                              size_t base_prime_count, unsigned char *is_prime,
...
static void *sieve_worker(void *arg) {
    ...
    unsigned char *is_prime =
        malloc((size_t)work->segment_odd_count * sizeof *is_prime);
    if (is_prime == NULL) {
    ...
    for (;;) {
        ...
        const uint64_t low_index =
            atomic_fetch_add(&work->next_odd_index, work->segment_odd_count);
        ...
        sieve_odd_segment(low_index, high_index, work->base_primes,
                          work->base_prime_count, is_prime, &segment_sum,
                          &segment_count);
